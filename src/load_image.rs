...
pub fn add_image_alpha_mut( img: &mut image::RgbaImage, path: &str, x: u32, y: u32, width: u32, height: u32, is_circle: bool) {
    ...
    if is_circle {
        ...
        for (draw_x, draw_y, pixel,) in circle_img.enumerate_pixels() {
            if pixel != &Rgba([0,0,0,0]) {
                ...
                let foreground = crate::utility::Rgba::new(pixel.0[0], pixel.0[1], pixel.0[2], pixel.0[3]);
                let background = crate::utility::Rgba::new(b_pixel.0[0], b_pixel.0[1], b_pixel.0[2], b_pixel.0[3]);
                let blend = crate::utility::Rgba::blend_alpha(&foreground, &background);
                super::rect::draw_rect(img,
                    draw_x + x,
                    draw_y + y,
                    1,
...
pub fn add_image_marge_mut( img: &mut image::RgbaImage, path: &str, x: u32, y: u32, width: u32, height: u32, is_circle: bool) {
